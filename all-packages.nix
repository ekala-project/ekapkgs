...
let
  ...
  collectVariants =
    ...
    if variants != null && isAttrs variants then
      let
        variantNames = tryAttr (attrNames variants);
      in
      if variantNames == null then
        ...
        concatLists (
          map (
            vname:
            let
              val = tryAttr variants.${vname};
            in
            if val != null && lib.isDerivation val then
              [
                {
                  name = "${name}-${vname}";
                  value = val;
