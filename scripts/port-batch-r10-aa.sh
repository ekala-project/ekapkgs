#!/usr/bin/env bash
# Port packages from nixpkgs to ekapkgs (r10 batch aa)
...
for pkg in "${PACKAGES[@]}"; do
  ...
  # Remove versionCheckHook from nativeInstallCheckInputs list (but keep other items)
  ...
  # Transform 6: Meson - add meson.configurePhaseHook and ensure ninja
  ...
  echo "  Evaluating $pkg..."
  EVAL_OUTPUT=$(nix-instantiate -A "$pkg" 2>&1)
  EVAL_EXIT=$?
  ...
  timeout 600 nix-build -A "$pkg" --no-out-link --timeout 600 2>&1 | tail -5
