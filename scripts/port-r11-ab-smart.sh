...
# Smart port: Transform, eval, check build size, then build smallest first
...
for pkg in "${PACKAGES[@]}"; do
  ...
  if nix-instantiate -A "$pkg" 2>/dev/null 1>/dev/null; then
    ...
    # Count how many derivations need to be built
    to_build=$(nix-build -A "$pkg" --dry-run 2>&1 | grep "will be built" | grep -oP '\d+' | head -1 || echo "0")
    [ -z "$to_build" ] && to_build=999
    echo "$to_build $pkg" >> /tmp/r11-ab-buildable.txt
  else
...
echo "Eval pass: $(wc -l < /tmp/r11-ab-buildable.txt)"
echo "Eval fail: $(wc -l < /tmp/r11-ab-fail-eval.txt)"
echo ""

echo "=== PHASE 2: Build (sorted by build size, smallest first) ==="
# Sort by number of derivations to build (smallest first)
sort -n /tmp/r11-ab-buildable.txt > /tmp/r11-ab-buildable-sorted.txt
...
while read -r line; do
  ...
  # Skip if already committed (may have been committed during this run)
  ...
  echo "Building $pkg (est. $count derivs to build)..."
...
done < /tmp/r11-ab-buildable-sorted.txt
