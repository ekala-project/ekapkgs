...
# Reads /tmp/r11-ac-eval-pass.txt
...
> /tmp/r11-ac-p2-skip.txt

mapfile -t PACKAGES < /tmp/r11-ac-eval-pass.txt
total=${#PACKAGES[@]}
idx=0
...
for pkg in "${PACKAGES[@]}"; do
  ...
  if git -C "$EKAPKGS" log --oneline --all --grep="${pkg}: init" -- "pkgs/${pkg}/" 2>/dev/null | grep -q .; then
    ...
    echo "$pkg" >> /tmp/r11-ac-p2-skip.txt
    continue
  fi
...
echo ""
echo "SKIPPED: $(wc -l < /tmp/r11-ac-p2-skip.txt)"
cat /tmp/r11-ac-p2-skip.txt
