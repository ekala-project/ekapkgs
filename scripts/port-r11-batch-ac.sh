#!/usr/bin/env bash
# Combined: copy, transform, eval, build, commit for all packages in r11-batch-ac
...
> /tmp/r11-ac-success.txt
> /tmp/r11-ac-fail-eval.txt
> /tmp/r11-ac-fail-build.txt
> /tmp/r11-ac-fail-timeout.txt
> /tmp/r11-ac-skip.txt
...
for pkg in "${PACKAGES[@]}"; do
  ...
  if [ ! -f "$src_file" ]; then
    ...
    echo "$pkg: no source" >> /tmp/r11-ac-skip.txt
    continue
  fi
  ...
  if git -C "$EKAPKGS" log --oneline --all --grep="${pkg}: init" -- "pkgs/${pkg}/" 2>/dev/null | grep -q .; then
    ...
    echo "$pkg: committed" >> /tmp/r11-ac-skip.txt
    continue
  fi
  ...
  if ! nix-instantiate -A "$pkg" 2>/dev/null >/dev/null; then
    ...
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r11-ac-fail-eval.txt
    continue
  fi
  ...
  if [ "$BUILD_EXIT" -eq 124 ]; then
    echo "TIMEOUT"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r11-ac-fail-timeout.txt
    continue
  elif [ "$BUILD_EXIT" -ne 0 ]; then
    echo "FAIL"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r11-ac-fail-build.txt
    continue
  fi
  ...
  echo "OK ($version)"
  echo "$pkg: $version" >> /tmp/r11-ac-success.txt
done
...
echo "FINAL RESULTS"
echo "================================================================"
echo "SUCCESS: $(wc -l < /tmp/r11-ac-success.txt)"
cat /tmp/r11-ac-success.txt
echo ""
echo "FAIL_EVAL: $(wc -l < /tmp/r11-ac-fail-eval.txt)"
cat /tmp/r11-ac-fail-eval.txt
echo ""
echo "FAIL_BUILD: $(wc -l < /tmp/r11-ac-fail-build.txt)"
cat /tmp/r11-ac-fail-build.txt
echo ""
echo "FAIL_TIMEOUT: $(wc -l < /tmp/r11-ac-fail-timeout.txt)"
cat /tmp/r11-ac-fail-timeout.txt
echo ""
echo "SKIPPED: $(wc -l < /tmp/r11-ac-skip.txt)"
cat /tmp/r11-ac-skip.txt
