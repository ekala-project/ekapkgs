...
echo "Eval pass: ${#eval_pass[@]}"
echo "Eval fail: $(wc -l < /tmp/r12-ab-fail-eval.txt)"
echo ""

echo "=== PHASE 2: Build (4 parallel jobs) ==="
# Write eval_pass to a file for parallel processing
> /tmp/r12-ab-eval-pass.txt
for pkg in "${eval_pass[@]}"; do
...
# Use xargs for parallel builds (4 at a time)
cat /tmp/r12-ab-eval-pass.txt | xargs -P4 -I{} bash -c 'build_one "$@"' _ {} > /tmp/r12-ab-build-results.txt

echo "=== PHASE 3: Commit successes ==="
while read -r result pkg; do
  dest_dir="$EKAPKGS/pkgs/${pkg}"
...
done < /tmp/r12-ab-build-results.txt
