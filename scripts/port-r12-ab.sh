...
# Port r12-batch-ab packages from nixpkgs to ekapkgs
...
apply_transforms() {
  ...
  # Complex multiline updateScript with _experimental-update-script-combinators
  perl -0777 -i -pe 's/\n\s*updateScript\s*=\s*_experimental-update-script-combinators[^;]*;\n/\n/gs' "$nix_file"
  # Catch-all for remaining updateScript patterns (multiline)
  perl -0777 -i -pe 's/\n\s*updateScript\s*=\s*(?:(?!;\n).)*;\n/\n/gs' "$nix_file"
...
for pkg in "${PACKAGES[@]}"; do
  ...
  if nix-instantiate -A "$pkg" 2>/dev/null 1>/dev/null; then
    ...
    to_build=$(nix-build -A "$pkg" --dry-run 2>&1 | grep -oP '\d+(?=\s+(derivation|path)s?\s+will\s+be\s+built)' | head -1 || echo "0")
    [ -z "$to_build" ] && to_build=999
    echo "$to_build $pkg" >> /tmp/r12-ab-buildable.txt
  else
...
echo "Eval pass: $(wc -l < /tmp/r12-ab-buildable.txt)"
echo "Eval fail: $(wc -l < /tmp/r12-ab-fail-eval.txt)"
echo ""

echo "=== PHASE 2: Build (sorted by build size, smallest first) ==="
sort -n /tmp/r12-ab-buildable.txt > /tmp/r12-ab-buildable-sorted.txt
...
done < /tmp/r12-ab-buildable-sorted.txt
