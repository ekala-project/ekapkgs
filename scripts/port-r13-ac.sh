#!/usr/bin/env bash
# Phase 1: Copy, transform, format, and eval-check all packages from r13-batch-ac
# Produces /tmp/r13-ac-eval-pass.txt and /tmp/r13-ac-eval-fail.txt
...
> /tmp/r13-ac-eval-pass.txt
> /tmp/r13-ac-eval-fail.txt
> /tmp/r13-ac-p1-skip.txt
> /tmp/r13-ac-success.txt
> /tmp/r13-ac-fail-build.txt
> /tmp/r13-ac-fail-timeout.txt

mapfile -t PACKAGES < /tmp/r13-batch-ac
total=${#PACKAGES[@]}
idx=0
...
for pkg in "${PACKAGES[@]}"; do
  ...
  if [ ! -f "$src_file" ]; then
    ...
    echo "$pkg" >> /tmp/r13-ac-p1-skip.txt
    continue
  fi
  ...
  if nix-instantiate -A "$pkg" 2>/dev/null >/dev/null; then
    ...
    echo "$pkg" >> /tmp/r13-ac-eval-pass.txt
    ...
    if [ "$BUILD_EXIT" -eq 124 ]; then
      echo "TIMEOUT"
      rm -rf "$dest_dir"
      echo "$pkg" >> /tmp/r13-ac-fail-timeout.txt
    elif [ "$BUILD_EXIT" -ne 0 ]; then
      echo "BUILD_FAIL"
      rm -rf "$dest_dir"
      echo "$pkg" >> /tmp/r13-ac-fail-build.txt
    else
      ...
      echo "OK ($version)"
      echo "$pkg: $version" >> /tmp/r13-ac-success.txt
    fi
  else
    echo "[$idx/$total] EVAL_FAIL: $pkg"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r13-ac-eval-fail.txt
  fi
done
...
echo "FINAL RESULTS"
echo "================================================================"
echo "SUCCESS: $(cat /tmp/r13-ac-success.txt 2>/dev/null | wc -l)"
cat /tmp/r13-ac-success.txt 2>/dev/null
echo ""
echo "EVAL_FAIL: $(wc -l < /tmp/r13-ac-eval-fail.txt)"
echo "BUILD_FAIL: $(cat /tmp/r13-ac-fail-build.txt 2>/dev/null | wc -l)"
echo "TIMEOUT: $(cat /tmp/r13-ac-fail-timeout.txt 2>/dev/null | wc -l)"
echo "SKIPPED: $(wc -l < /tmp/r13-ac-p1-skip.txt)"
