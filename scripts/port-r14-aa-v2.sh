#!/usr/bin/env bash
# Port r14-batch-aa packages from nixpkgs to ekapkgs - v2 with reliable commits
set -uo pipefail
...
> /tmp/r14-aa-v2-success.txt
> /tmp/r14-aa-v2-eval-fail.txt
> /tmp/r14-aa-v2-build-fail.txt
> /tmp/r14-aa-v2-timeout.txt
> /tmp/r14-aa-v2-skip.txt
...
for pkg in "${PACKAGES[@]}"; do
  ...
  if [ ! -f "$src_file" ]; then
    ...
    echo "$pkg" >> /tmp/r14-aa-v2-skip.txt
    continue
  fi
  ...
  if ! nix-instantiate -A "$pkg" 2>/dev/null >/dev/null; then
    ...
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r14-aa-v2-eval-fail.txt
    continue
  fi
  ...
  if ! timeout 660 nix-build -A "$pkg" --no-out-link --timeout 600 2>/dev/null >/dev/null; then
    echo "BUILD_FAIL"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r14-aa-v2-build-fail.txt
    continue
  fi
  ...
  if git diff --cached --quiet "pkgs/${pkg}/" 2>/dev/null; then
    # Nothing staged - try without trailing slash
    git add "pkgs/${pkg}"
  fi

  if git commit -m "${pkg}: init at ${version}"; then
    echo "OK ($version)"
    echo "$pkg: $version" >> /tmp/r14-aa-v2-success.txt
  else
...
echo "FINAL RESULTS"
echo "================================================================"
echo "SUCCESS: $(wc -l < /tmp/r14-aa-v2-success.txt)"
cat /tmp/r14-aa-v2-success.txt
echo ""
echo "EVAL_FAIL: $(wc -l < /tmp/r14-aa-v2-eval-fail.txt)"
echo "BUILD_FAIL: $(wc -l < /tmp/r14-aa-v2-build-fail.txt)"
echo "TIMEOUT: $(wc -l < /tmp/r14-aa-v2-timeout.txt)"
echo "SKIPPED: $(wc -l < /tmp/r14-aa-v2-skip.txt)"
