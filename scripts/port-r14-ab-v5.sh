...
RESULTS="/tmp/r14-ab-results-v5.log"
...
git_locked_commit() {
    ...
    for i in $(seq 1 60); do
        if mkdir "$LOCKFILE" 2>/dev/null; then
            ...
            local result=0
            git commit -m "$PKG: init at $VERSION" 2>/dev/null || result=$?
...
process_one() {
    ...
    # cmake hook - handle various formats
    ...
    if git_locked_commit "$PKG" "$VERSION"; then
