#!/usr/bin/env bash
# Phase 1: Copy, transform, format, and eval-check all packages from r14-batch-ac
# Produces /tmp/r14-ac-eval-pass.txt and /tmp/r14-ac-eval-fail.txt
...
> /tmp/r14-ac-eval-pass.txt
> /tmp/r14-ac-eval-fail.txt
> /tmp/r14-ac-p1-skip.txt
> /tmp/r14-ac-success.txt
> /tmp/r14-ac-fail-build.txt
> /tmp/r14-ac-fail-timeout.txt

mapfile -t PACKAGES < /tmp/r14-batch-ac
total=${#PACKAGES[@]}
idx=0
...
for pkg in "${PACKAGES[@]}"; do
  ...
  if [ ! -f "$src_file" ]; then
    ...
    echo "$pkg" >> /tmp/r14-ac-p1-skip.txt
    continue
  fi
  ...
  # Remove updateScript assignments (handle nested braces)
  ...
  # Handle gnome.updateScript, lib.* updateScript, and other function-call forms with nested braces
  perl -0777 -i -pe 's/\s*updateScript\s*=\s*\w+[\w.]*\s*(\{(?:[^{}]|\{(?:[^{}]|\{[^{}]*\})*\})*\});\n?//gs' "$nix_file"
  # Catch-all: remove any remaining single-line updateScript = ...;
  ...
  # Remove passthru.tests that only had version check
  perl -0777 -i -pe 's/\s*passthru\.tests\s*=\s*\{\s*version\s*=\s*testers\.testVersion\s*\{[^}]*\};\s*\};\n?//gs' "$nix_file"
  # Remove passthru.tests.version inside passthru block
  perl -0777 -i -pe 's/\s*tests\s*=\s*\{\s*version\s*=\s*testers\.testVersion\s*(\{(?:[^{}]|\{(?:[^{}]|\{[^{}]*\})*\})*\});\s*\};\n?//gs' "$nix_file"
  # Remove tests block that only had nixosTests or is now empty
  perl -0777 -i -pe 's/\s*tests\s*=\s*\{\s*\};\n?//gs' "$nix_file"
  ...
  # Go packages: if buildGoModule is used and the package needs Go >= 1.26, use buildGo126Module
  if grep -q '\bbuildGoModule\b' "$nix_file" && ! grep -q 'buildGo126Module' "$nix_file"; then
    sed -i 's/\bbuildGoModule\b/buildGo126Module/g' "$nix_file"
  fi

  # Remove empty passthru (various whitespace forms)
  perl -0777 -i -pe 's/\n\s*passthru\s*=\s*\{\s*\};\n/\n/gs' "$nix_file"
  perl -0777 -i -pe 's/\n\s*passthru\s*=\s*\{\s*\n\s*\};\n/\n/gs' "$nix_file"
  # Remove passthru with only empty tests block
  perl -0777 -i -pe 's/\n\s*passthru\s*=\s*\{\s*\n\s*tests\s*=\s*\{\s*\};\s*\n\s*\};\n/\n/gs' "$nix_file"
  # Remove testers import if only used for passthru.tests.version that was removed
  if ! grep -q 'testers' "$nix_file" || ( ! grep -q 'testers\.' "$nix_file" ); then
  ...
  if nix-instantiate -A "$pkg" 2>/dev/null >/dev/null; then
    ...
    echo "$pkg" >> /tmp/r14-ac-eval-pass.txt
    ...
    if [ "$BUILD_EXIT" -eq 124 ]; then
      echo "TIMEOUT"
      rm -rf "$dest_dir"
      echo "$pkg" >> /tmp/r14-ac-fail-timeout.txt
    elif [ "$BUILD_EXIT" -ne 0 ]; then
      echo "BUILD_FAIL"
      rm -rf "$dest_dir"
      echo "$pkg" >> /tmp/r14-ac-fail-build.txt
    else
      ...
      echo "OK ($version)"
      echo "$pkg: $version" >> /tmp/r14-ac-success.txt
    fi
  else
    echo "[$idx/$total] EVAL_FAIL: $pkg"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r14-ac-eval-fail.txt
  fi
done
...
echo "FINAL RESULTS"
echo "================================================================"
echo "SUCCESS: $(cat /tmp/r14-ac-success.txt 2>/dev/null | wc -l)"
cat /tmp/r14-ac-success.txt 2>/dev/null
echo ""
echo "EVAL_FAIL: $(wc -l < /tmp/r14-ac-eval-fail.txt)"
echo "BUILD_FAIL: $(cat /tmp/r14-ac-fail-build.txt 2>/dev/null | wc -l)"
echo "TIMEOUT: $(cat /tmp/r14-ac-fail-timeout.txt 2>/dev/null | wc -l)"
echo "SKIPPED: $(wc -l < /tmp/r14-ac-p1-skip.txt)"
