#!/usr/bin/env bash
# Combined port script for r15-batch-aa packages
# Creates dir, applies transforms, formats, evals, builds, commits - all in one pass
set -uo pipefail
...
> /tmp/r15-aa-c-success.txt
> /tmp/r15-aa-c-eval-fail.txt
> /tmp/r15-aa-c-build-fail.txt
> /tmp/r15-aa-c-skip.txt
...
apply_transforms() {
  ...
  # Remove ALL updater scripts from inputs
...
for pkg in "${PACKAGES[@]}"; do
  ...
  if [ ! -f "$src_file" ]; then
    ...
    echo "$pkg: no_source" >> /tmp/r15-aa-c-skip.txt
    continue
  fi
  ...
  if git log --oneline --all --grep="${pkg}: init" -- "pkgs/${pkg}/" 2>/dev/null | grep -q .; then
    ...
    echo "$pkg: committed" >> /tmp/r15-aa-c-skip.txt
    continue
  fi
  ...
  if ! timeout 30 nix-instantiate -A "$pkg" 2>/dev/null >/dev/null; then
    ...
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r15-aa-c-eval-fail.txt
    continue
  fi
  ...
  if ! timeout 660 nix-build -A "$pkg" --no-out-link --timeout 600 2>/dev/null >/dev/null; then
    echo "BUILD_FAIL"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r15-aa-c-build-fail.txt
    continue
  fi
  ...
  if git commit -m "${pkg}: init at ${version}"; then
    echo "OK ($version)"
    echo "$pkg: $version" >> /tmp/r15-aa-c-success.txt
  else
...
echo "FINAL RESULTS"
echo "================================================================"
echo "SUCCESS: $(wc -l < /tmp/r15-aa-c-success.txt)"
cat /tmp/r15-aa-c-success.txt
echo ""
echo "EVAL_FAIL: $(wc -l < /tmp/r15-aa-c-eval-fail.txt)"
echo "BUILD_FAIL: $(wc -l < /tmp/r15-aa-c-build-fail.txt)"
echo "SKIPPED: $(wc -l < /tmp/r15-aa-c-skip.txt)"
