#!/usr/bin/env bash
# Phase 1: Eval-only pass for r15-batch-aa packages
# Creates package dirs, applies transforms, tests eval
# Outputs list of packages that pass eval to /tmp/r15-aa-eval-pass.txt
set -uo pipefail
...
> /tmp/r15-aa-eval-pass.txt
> /tmp/r15-aa-p1-eval-fail.txt
> /tmp/r15-aa-p1-skip.txt

apply_transforms() {
  local nix_file="$1"

  # Maintainers -> empty list (multiple patterns)
  ...
  # Remove update scripts from inputs (all known updaters)
  ...
  perl -0777 -i -pe 's/\s*tests\.pkg-config\s*=\s*testers\.testMetaPkgConfig;\n?//gs' "$nix_file"
  perl -0777 -i -pe 's/\s*tests\.testers\s*=\s*[^;]*;\n?//gs' "$nix_file"
...
mapfile -t PACKAGES < /tmp/r15-batch-aa
total=${#PACKAGES[@]}
idx=0

# First remove packages already successfully committed
for pkg in "${PACKAGES[@]}"; do
  ...
  if [ ! -f "$src_file" ]; then
    ...
    echo "$pkg: no_source" >> /tmp/r15-aa-p1-skip.txt
    continue
  fi
  ...
  if git log --oneline --all --grep="${pkg}: init" -- "pkgs/${pkg}/" 2>/dev/null | grep -q .; then
    ...
    echo "$pkg: committed" >> /tmp/r15-aa-p1-skip.txt
    continue
  fi
  ...
  # Eval
  if timeout 30 nix-instantiate -A "$pkg" 2>/dev/null >/dev/null; then
    echo "[$idx/$total] EVAL_OK: $pkg"
    echo "$pkg" >> /tmp/r15-aa-eval-pass.txt
  else
    echo "[$idx/$total] EVAL_FAIL: $pkg"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r15-aa-p1-eval-fail.txt
  fi
done
...
echo "================================================================"
echo "EVAL_PASS: $(wc -l < /tmp/r15-aa-eval-pass.txt)"
echo "EVAL_FAIL: $(wc -l < /tmp/r15-aa-p1-eval-fail.txt)"
echo "SKIPPED: $(wc -l < /tmp/r15-aa-p1-skip.txt)"
echo ""
echo "Packages that pass eval:"
cat /tmp/r15-aa-eval-pass.txt
