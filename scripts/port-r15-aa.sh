#!/usr/bin/env bash
# Port r15-batch-aa packages from nixpkgs to ekapkgs
set -uo pipefail
...
> /tmp/r15-aa-success.txt
> /tmp/r15-aa-eval-fail.txt
> /tmp/r15-aa-build-fail.txt
> /tmp/r15-aa-timeout.txt
> /tmp/r15-aa-skip.txt
...
for pkg in "${PACKAGES[@]}"; do
  ...
  if ! nix-instantiate -A "$pkg" 2>/dev/null >/dev/null; then
    ...
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r15-aa-eval-fail.txt
    continue
  fi
  ...
  if ! timeout 660 nix-build -A "$pkg" --no-out-link --timeout 600 2>/dev/null >/dev/null; then
    echo "BUILD_FAIL"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r15-aa-build-fail.txt
    continue
  fi
  ...
  if git commit -m "${pkg}: init at ${version}"; then
    echo "OK ($version)"
    echo "$pkg: $version" >> /tmp/r15-aa-success.txt
  else
...
echo "FINAL RESULTS"
echo "================================================================"
echo "SUCCESS: $(wc -l < /tmp/r15-aa-success.txt)"
cat /tmp/r15-aa-success.txt
echo ""
echo "EVAL_FAIL: $(wc -l < /tmp/r15-aa-eval-fail.txt)"
cat /tmp/r15-aa-eval-fail.txt
echo ""
echo "BUILD_FAIL: $(wc -l < /tmp/r15-aa-build-fail.txt)"
cat /tmp/r15-aa-build-fail.txt
echo ""
echo "TIMEOUT: $(wc -l < /tmp/r15-aa-timeout.txt)"
echo "SKIPPED: $(wc -l < /tmp/r15-aa-skip.txt)"
cat /tmp/r15-aa-skip.txt
