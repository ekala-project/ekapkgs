#!/usr/bin/env bash
# Port r16-batch-aa packages from nixpkgs to ekapkgs - v2 with better transforms
set -uo pipefail
...
> /tmp/r16-aa-v2-success.txt
> /tmp/r16-aa-v2-eval-fail.txt
> /tmp/r16-aa-v2-build-fail.txt
> /tmp/r16-aa-v2-skip.txt
...
apply_transforms() {
  ...
  sed -i '/^\s*writeShellScript,\?$/d' "$nix_file" 2>/dev/null || true
  sed -i '/passthru\.updateScript/d' "$nix_file"

  # Remove updateScript assignments (multi-line aware - many patterns)
  ...
  # Remove versionCheckHook (all patterns)
  ...
  # Remove testers (all patterns including testMetaPkgConfig)
  perl -0777 -i -pe 's/\s*tests\.version\s*=\s*testers\.testVersion\s*\{[^}]*\}\s*;\n?//gs' "$nix_file"
  perl -0777 -i -pe 's/\s*tests\.version\s*=\s*testers\.testVersion\s*;\n?//gs' "$nix_file"
  perl -0777 -i -pe 's/\s*tests\.pkg-config\s*=\s*testers\.testMetaPkgConfig\s+[^;]*;\n?//gs' "$nix_file"
...
for pkg in "${PACKAGES[@]}"; do
  ...
  if [ ! -f "$src_file" ]; then
    ...
    echo "$pkg (no source)" >> /tmp/r16-aa-v2-skip.txt
    continue
  fi
  ...
  if git log --oneline --all --grep="${pkg}: init" -- "pkgs/${pkg}/" 2>/dev/null | grep -q .; then
    ...
    echo "$pkg" >> /tmp/r16-aa-v2-skip.txt
    continue
  fi

  # Skip if dir exists (from another process)
  if [ -d "$dest_dir" ]; then
    echo "[$idx/$total] SKIP(exists): $pkg"
    echo "$pkg (exists)" >> /tmp/r16-aa-v2-skip.txt
    continue
  fi
  ...
  if ! nix-instantiate -A "$pkg" 2>/dev/null >/dev/null; then
    ...
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r16-aa-v2-eval-fail.txt
    continue
  fi
  ...
  if ! timeout 660 nix-build -A "$pkg" --no-out-link --timeout 600 2>/dev/null >/dev/null; then
    echo "BUILD_FAIL"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r16-aa-v2-build-fail.txt
    continue
  fi
  ...
  if git commit -m "${pkg}: init at ${version}"; then
    echo "OK ($version)"
    echo "$pkg: $version" >> /tmp/r16-aa-v2-success.txt
  else
...
echo "FINAL RESULTS"
echo "================================================================"
echo "SUCCESS: $(wc -l < /tmp/r16-aa-v2-success.txt)"
cat /tmp/r16-aa-v2-success.txt
echo ""
echo "EVAL_FAIL: $(wc -l < /tmp/r16-aa-v2-eval-fail.txt)"
cat /tmp/r16-aa-v2-eval-fail.txt
echo ""
echo "BUILD_FAIL: $(wc -l < /tmp/r16-aa-v2-build-fail.txt)"
cat /tmp/r16-aa-v2-build-fail.txt
echo ""
echo "SKIPPED: $(wc -l < /tmp/r16-aa-v2-skip.txt)"
cat /tmp/r16-aa-v2-skip.txt
