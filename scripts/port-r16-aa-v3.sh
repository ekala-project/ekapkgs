#!/usr/bin/env bash
# Port r16-batch-aa packages - v3: eval-first then build
set -uo pipefail
...
> /tmp/r16-aa-v3-success.txt
> /tmp/r16-aa-v3-eval-fail.txt
> /tmp/r16-aa-v3-build-fail.txt
> /tmp/r16-aa-v3-skip.txt
> /tmp/r16-aa-v3-eval-pass.txt
...
apply_transforms() {
  ...
  # Remove updater scripts from inputs (all known patterns)
  ...
  # Remove empty passthru blocks (multiple patterns)
  perl -0777 -i -pe 's/\n\s*passthru\s*=\s*\{\s*\};\n/\n/gs' "$nix_file"
...
# Read remaining packages
mapfile -t PACKAGES < /tmp/r16-batch-aa
total=${#PACKAGES[@]}

echo "=== PHASE 1: Eval all packages ==="
idx=0
for pkg in "${PACKAGES[@]}"; do
  ...
  if [ ! -f "$src_file" ]; then
    ...
    echo "$pkg (no source)" >> /tmp/r16-aa-v3-skip.txt
    continue
  fi
  ...
  if [ -d "$dest_dir" ] && git log --oneline -1 --grep="${pkg}: init" -- "pkgs/${pkg}/" 2>/dev/null | grep -q .; then
    ...
    echo "$pkg" >> /tmp/r16-aa-v3-skip.txt
    continue
  fi
  ...
  if ! nix-instantiate -A "$pkg" 2>/dev/null >/dev/null; then
    ...
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r16-aa-v3-eval-fail.txt
    continue
  fi

  echo "[$idx/$total] EVAL_OK: $pkg"
  echo "$pkg" >> /tmp/r16-aa-v3-eval-pass.txt
done

echo ""
echo "=== PHASE 1 DONE: $(wc -l < /tmp/r16-aa-v3-eval-pass.txt) packages pass eval ==="
echo ""

echo "=== PHASE 2: Build packages ==="
bidx=0
btotal=$(wc -l < /tmp/r16-aa-v3-eval-pass.txt)
...
while IFS= read -r pkg; do
  ...
  if ! timeout 660 nix-build -A "$pkg" --no-out-link --timeout 600 2>/dev/null >/dev/null; then
    echo "BUILD_FAIL"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r16-aa-v3-build-fail.txt
    continue
  fi
  ...
  if git commit -m "${pkg}: init at ${version}"; then
    echo "OK ($version)"
    echo "$pkg: $version" >> /tmp/r16-aa-v3-success.txt
  else
    echo "COMMIT_FAIL ($version)"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r16-aa-v3-build-fail.txt
  fi
done < /tmp/r16-aa-v3-eval-pass.txt

echo ""
echo "================================================================"
echo "FINAL RESULTS"
echo "================================================================"
echo "SUCCESS: $(wc -l < /tmp/r16-aa-v3-success.txt)"
cat /tmp/r16-aa-v3-success.txt
echo ""
echo "EVAL_FAIL: $(wc -l < /tmp/r16-aa-v3-eval-fail.txt)"
cat /tmp/r16-aa-v3-eval-fail.txt
echo ""
echo "BUILD_FAIL: $(wc -l < /tmp/r16-aa-v3-build-fail.txt)"
cat /tmp/r16-aa-v3-build-fail.txt
echo ""
echo "SKIPPED: $(wc -l < /tmp/r16-aa-v3-skip.txt)"
cat /tmp/r16-aa-v3-skip.txt
