#!/usr/bin/env bash
# Port r17-batch-aa packages - eval-first then build
set -uo pipefail
...
> /tmp/r17-aa-success.txt
> /tmp/r17-aa-eval-fail.txt
> /tmp/r17-aa-build-fail.txt
> /tmp/r17-aa-skip.txt
> /tmp/r17-aa-eval-pass.txt
...
apply_transforms() {
  ...
  # meson: add configurePhaseHook + ensure ninja
...
# Read packages
mapfile -t PACKAGES < /tmp/r17-batch-aa
total=${#PACKAGES[@]}
...
for pkg in "${PACKAGES[@]}"; do
  ...
  if [ ! -f "$src_file" ]; then
    ...
    echo "$pkg (no source)" >> /tmp/r17-aa-skip.txt
    continue
  fi
  ...
  if ! nix-instantiate -A "$pkg" 2>/dev/null >/dev/null; then
    ...
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r17-aa-eval-fail.txt
    continue
  fi

  echo "[$idx/$total] EVAL_OK: $pkg"
  echo "$pkg" >> /tmp/r17-aa-eval-pass.txt
done

echo ""
echo "=== PHASE 1 DONE: $(wc -l < /tmp/r17-aa-eval-pass.txt) packages pass eval ==="
echo ""

echo "=== PHASE 2: Build packages ==="
bidx=0
btotal=$(wc -l < /tmp/r17-aa-eval-pass.txt)
...
while IFS= read -r pkg; do
  ...
  if ! timeout 660 nix-build -A "$pkg" --no-out-link --timeout 600 2>/dev/null >/dev/null; then
    echo "BUILD_FAIL"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r17-aa-build-fail.txt
    continue
  fi
  ...
  if git commit -m "${pkg}: init at ${version}"; then
    echo "OK ($version)"
    echo "$pkg: $version" >> /tmp/r17-aa-success.txt
  else
    echo "COMMIT_FAIL ($version)"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r17-aa-build-fail.txt
  fi
done < /tmp/r17-aa-eval-pass.txt

echo ""
echo "================================================================"
echo "FINAL RESULTS"
echo "================================================================"
echo "SUCCESS: $(wc -l < /tmp/r17-aa-success.txt)"
cat /tmp/r17-aa-success.txt
echo ""
echo "EVAL_FAIL: $(wc -l < /tmp/r17-aa-eval-fail.txt)"
cat /tmp/r17-aa-eval-fail.txt
echo ""
echo "BUILD_FAIL: $(wc -l < /tmp/r17-aa-build-fail.txt)"
cat /tmp/r17-aa-build-fail.txt
echo ""
echo "SKIPPED: $(wc -l < /tmp/r17-aa-skip.txt)"
cat /tmp/r17-aa-skip.txt
