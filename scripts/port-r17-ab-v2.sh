#!/usr/bin/env bash
# Port r17-batch-ab packages - v2: with Go 1.26 support, smarter transforms
set -uo pipefail
...
> /tmp/r17-ab-v2-success.txt
> /tmp/r17-ab-v2-eval-fail.txt
> /tmp/r17-ab-v2-build-fail.txt
> /tmp/r17-ab-v2-skip.txt
> /tmp/r17-ab-v2-eval-pass.txt
...
echo "=== PHASE 1: Eval all $total packages ==="
idx=0
for pkg in "${PACKAGES[@]}"; do
  ...
  if [ ! -f "$src_file" ]; then
    ...
    echo "$pkg (no source)" >> /tmp/r17-ab-v2-skip.txt
    continue
  fi

  # Skip if already committed
  if [ -d "$dest_dir" ] && git log --oneline -1 --grep="${pkg}: init" -- "pkgs/${pkg}/" 2>/dev/null | grep -q .; then
    echo "[$idx/$total] SKIP(committed): $pkg"
    echo "$pkg" >> /tmp/r17-ab-v2-skip.txt
    continue
  fi
  ...
  if ! nix-instantiate -A "$pkg" 2>/dev/null >/dev/null; then
    ...
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r17-ab-v2-eval-fail.txt
    continue
  fi

  echo "[$idx/$total] EVAL_OK: $pkg"
  echo "$pkg" >> /tmp/r17-ab-v2-eval-pass.txt
done

echo ""
echo "=== PHASE 1 DONE: $(wc -l < /tmp/r17-ab-v2-eval-pass.txt) packages pass eval ==="
echo ""

echo "=== PHASE 2: Build packages ==="
bidx=0
btotal=$(wc -l < /tmp/r17-ab-v2-eval-pass.txt)
...
while IFS= read -r pkg; do
  ...
  if [ $build_rc -ne 0 ]; then
    # Check if it's a Go version issue we missed
    if echo "$build_output" | grep -q "go.mod requires go >= 1.26"; then
      # Already should have been handled but try again
      echo "GO_VER_FAIL (already converted)"
    fi
    echo "BUILD_FAIL"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r17-ab-v2-build-fail.txt
    continue
  fi
  ...
  if git commit -m "${pkg}: init at ${version}"; then
    echo "OK ($version)"
    echo "$pkg: $version" >> /tmp/r17-ab-v2-success.txt
  else
    echo "COMMIT_FAIL ($version)"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r17-ab-v2-build-fail.txt
  fi
done < /tmp/r17-ab-v2-eval-pass.txt

echo ""
echo "================================================================"
echo "FINAL RESULTS"
echo "================================================================"
echo "SUCCESS: $(wc -l < /tmp/r17-ab-v2-success.txt)"
cat /tmp/r17-ab-v2-success.txt
echo ""
echo "EVAL_FAIL: $(wc -l < /tmp/r17-ab-v2-eval-fail.txt)"
cat /tmp/r17-ab-v2-eval-fail.txt
echo ""
echo "BUILD_FAIL: $(wc -l < /tmp/r17-ab-v2-build-fail.txt)"
cat /tmp/r17-ab-v2-build-fail.txt
echo ""
echo "SKIPPED: $(wc -l < /tmp/r17-ab-v2-skip.txt)"
cat /tmp/r17-ab-v2-skip.txt
