#!/usr/bin/env bash
# Port r17-batch-ab packages - two-phase: eval first, then build
set -uo pipefail
...
> /tmp/r17-ab-success.txt
> /tmp/r17-ab-eval-fail.txt
> /tmp/r17-ab-build-fail.txt
> /tmp/r17-ab-skip.txt
> /tmp/r17-ab-eval-pass.txt
...
apply_transforms() {
  ...
  # cmake: add configurePhaseHook (handle cmake.v4 for cmake >= 4.x packages)
...
for pkg in "${PACKAGES[@]}"; do
  ...
  if [ ! -f "$src_file" ]; then
    ...
    echo "$pkg (no source)" >> /tmp/r17-ab-skip.txt
    continue
  fi
  ...
  if ! nix-instantiate -A "$pkg" 2>/dev/null >/dev/null; then
    ...
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r17-ab-eval-fail.txt
    continue
  fi

  echo "[$idx/$total] EVAL_OK: $pkg"
  echo "$pkg" >> /tmp/r17-ab-eval-pass.txt
done

echo ""
echo "=== PHASE 1 DONE: $(wc -l < /tmp/r17-ab-eval-pass.txt) packages pass eval ==="
echo ""

echo "=== PHASE 2: Build packages ==="
bidx=0
btotal=$(wc -l < /tmp/r17-ab-eval-pass.txt)
...
while IFS= read -r pkg; do
  ...
  if ! timeout 660 nix-build -A "$pkg" --no-out-link --timeout 600 2>/dev/null >/dev/null; then
    echo "BUILD_FAIL"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r17-ab-build-fail.txt
    continue
  fi
  ...
  if git commit -m "${pkg}: init at ${version}"; then
    echo "OK ($version)"
    echo "$pkg: $version" >> /tmp/r17-ab-success.txt
  else
    echo "COMMIT_FAIL ($version)"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r17-ab-build-fail.txt
  fi
done < /tmp/r17-ab-eval-pass.txt

echo ""
echo "================================================================"
echo "FINAL RESULTS"
echo "================================================================"
echo "SUCCESS: $(wc -l < /tmp/r17-ab-success.txt)"
cat /tmp/r17-ab-success.txt
echo ""
echo "EVAL_FAIL: $(wc -l < /tmp/r17-ab-eval-fail.txt)"
cat /tmp/r17-ab-eval-fail.txt
echo ""
echo "BUILD_FAIL: $(wc -l < /tmp/r17-ab-build-fail.txt)"
cat /tmp/r17-ab-build-fail.txt
echo ""
echo "SKIPPED: $(wc -l < /tmp/r17-ab-skip.txt)"
cat /tmp/r17-ab-skip.txt
