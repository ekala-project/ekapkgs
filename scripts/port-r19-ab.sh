#!/usr/bin/env bash
# Port r19-batch-ab packages from nixpkgs to ekapkgs
set -uo pipefail
...
> /tmp/r19-ab-success.txt
> /tmp/r19-ab-eval-fail.txt
> /tmp/r19-ab-build-fail.txt
> /tmp/r19-ab-skip.txt
...
mapfile -t PACKAGES < /tmp/r19-batch-ab
total=${#PACKAGES[@]}
...
for pkg in "${PACKAGES[@]}"; do
  ...
  if [ ! -f "$src_dir/package.nix" ]; then
    ...
    echo "$pkg (no source)" >> /tmp/r19-ab-skip.txt
    continue
  fi
  ...
  if [ -z "$drv" ]; then
    ...
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r19-ab-eval-fail.txt
    continue
  fi
  ...
  if ! timeout 660 nix-build -A "$pkg" --no-out-link --timeout 600 2>/dev/null >/dev/null; then
    echo "BUILD_FAIL"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r19-ab-build-fail.txt
    continue
  fi
  ...
  if git commit -m "${pkg}: init at ${version}"; then
    echo "OK ($version)"
    echo "$pkg: $version" >> /tmp/r19-ab-success.txt
  else
    echo "COMMIT_FAIL"
    rm -rf "$dest_dir"
    echo "$pkg" >> /tmp/r19-ab-build-fail.txt
...
echo "FINAL RESULTS"
echo "================================================================"
echo "SUCCESS: $(wc -l < /tmp/r19-ab-success.txt)"
cat /tmp/r19-ab-success.txt
echo ""
echo "EVAL_FAIL: $(wc -l < /tmp/r19-ab-eval-fail.txt)"
cat /tmp/r19-ab-eval-fail.txt
echo ""
echo "BUILD_FAIL: $(wc -l < /tmp/r19-ab-build-fail.txt)"
cat /tmp/r19-ab-build-fail.txt
echo ""
echo "SKIPPED: $(wc -l < /tmp/r19-ab-skip.txt)"
cat /tmp/r19-ab-skip.txt
